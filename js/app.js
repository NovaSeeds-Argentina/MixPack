...
function openGallery(folderPath, fromHistory=false){
  ...
  // Extras por coleccion (agregar al final, al lado de Front2.jpg)
  if (folderPath.indexOf('02Genext/01') !== -1) {
    images.push(`${folderPath}/x2.png`);
  }
  if (folderPath.indexOf('02Genext/02') !== -1) {
    images.push(`${folderPath}/x1.png`);
  }
...
function buildFicha(meta, titleOverride){
  ...
  d.innerHTML = `
    ...
    <div class="ficha-grid">
      ...
      ${meta.notas ? `<div class=\"notas\"><b>Notas</b><span>${meta.notas}</span></div>` : ''}
    </div>`;
  return d;
}
...
// vAutoGranel: completar ficha técnica (Carrusel 2 · Tarjeta 3)
(function(){
  try{
    window.DATA_OVERRIDE = window.DATA_OVERRIDE || DATA_OVERRIDE || {};
    if(!DATA_OVERRIDE["02Genext"]) DATA_OVERRIDE["02Genext"] = {};
